...
export const createProfile = async (name, timezone = 'America/New_York') => {
  const response = await axios.post(`${BACKEND_API_URL}/users`, { name, timezone });
  return response.data;
};

export const updateProfileTimezone = async (id, timezone) => {
  const response = await axios.patch(`${BACKEND_API_URL}/users/${id}`, { timezone });
  return response.data;
};
...
export const getEventsByProfile = async (profileId) => {
  const response = await axios.get(`${BACKEND_API_URL}/events/profile/${profileId}`);
  return response.data;
};

export const createEvent = async (eventData) => {
  const response = await axios.post(`${BACKEND_API_URL}/events`, eventData);
  return response.data;
};

export const updateEvent = async (id, eventData) => {
  const response = await axios.put(`${BACKEND_API_URL}/events/${id}`, eventData);
  return response.data;
};
